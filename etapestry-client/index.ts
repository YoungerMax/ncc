...
export class EtApiClient {
  ...
  private wsdlEndpoint: string;

  constructor(wsdlEndpoint: string, timeoutMs: number = 15000) {
  ...
  private buildElement(tag: string, type: string, value: any): string {
    ...
    if (type.startsWith("tns:")) {
      ...
      // arrays
      const arrAttr =
        wsdlType.complexContent?.restriction.attribute["@_wsdl:arrayType"] ||
        "";

      if (arrAttr) {
        const itemType = arrAttr.replace(/\[\]$/, "");
        const items = (value as any[])
          .map((v) => this.buildElement("item", itemType, v))
          .join("");
      ...
      // object
      const elems =
        wsdlType.sequence?.element
          .map((el) => {
            return this.buildElement(
              el["@_name"],
              el["@_type"],
              (value as any)[el["@_name"]],
            );
          })
          .join("") || "";
...
export class EtApiSimpleClient {
  ...
  static async login(
    ...
    while (true) {
      ...
      const resp = await c.call("apiKeyLogin", [databaseId, apiKey]);
      let result = resp?.["ns0:apiKeyLoginResponse"]?.["result"]?.["#text"];
  ...
  async getJournalEntires(accountRef: string, start: number, count: number) {
    return this.client.call("getJournalEntries", [
      {
        count,
        accountRef,
        start,
